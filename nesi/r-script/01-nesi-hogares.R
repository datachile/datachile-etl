...
hogares <- list.files("raw-data", pattern = glob2rx("*hogares*.sav"), full.names = T, recursive = T)
hogares <- hogares[grepl("sin", hogares) == F]

hogares <- map(hogares, .f = haven::read_sav)
hogares <- map(hogares, .f = janitor::clean_names)

ultima_nesi <- hogares[[7]]
...
for (i in 1:length(hogares)) {
  ...
  write_csv(
    hogares[[i]], sprintf("clean-data/hogares/nesi-hogares-con-subsidios-%s.csv", years[i])
  )
}
