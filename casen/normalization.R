...
region_codes <- as.data.frame(lapply(region_codes, function(x) gsub("Biobío", "B\u00edob\u00edo", x)))
region_codes <- as.data.frame(lapply(region_codes, function(x) gsub("Alto Bio Bio", "Alto B\u00edob\u00edo", x)))
...
provincias_codes <- c("151:Arica","152:Parinacota","11:Iquique","12:Tamarugal","21:Antofagasta",
...
provincias_codes <- as.data.frame(provincias_codes)
provincias_codes <- data.frame(do.call('rbind', strsplit(as.character(provincias_codes$provincias_codes), ':', fixed=TRUE)))
setnames(provincias_codes, colnames(provincias_codes), c("provincia_pacha_id","provincia_name"))