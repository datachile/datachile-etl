########
# 1996 #
########

# Median Wage by Gender by region
median_1996_region <- aggregate(ingreso_ocup_principal ~ sexo + region, data = wage_by_gender_1996, FUN = median, na.rm = TRUE)
setnames(median_1996_region, colnames(median_1996_region), c("sexo","region","1996"))
#median_1996_region <- insert_row(median_1996_region, c("Pa\u00eds", median(wage_by_gender_1996$ingreso_ocup_principal, na.rm = TRUE)), nrow(median_1996_region)+1)
median_1996_region$`1996` <- as.numeric(median_1996_region$`1996`)
median_1996_region$`1996` <- round(median_1996_region$`1996`, 0)
median_1996_region$`1996` <- as.numeric(median_1996_region$`1996`)
median_1996_region$`1996` <- round(median_1996_region$`1996`, 0)

# Average Wage by Gender by region
mean_1996_region <- aggregate(ingreso_ocup_principal ~ sexo + region, data = wage_by_gender_1996, FUN = mean, na.rm = TRUE)
setnames(mean_1996_region, colnames(mean_1996_region), c("sexo","region","1996"))
mean_1996_region$`1996` <- as.numeric(mean_1996_region$`1996`)
mean_1996_region$`1996` <- round(mean_1996_region$`1996`, 0)
mean_1996_region$`1996` <- as.numeric(mean_1996_region$`1996`)
mean_1996_region$`1996` <- round(mean_1996_region$`1996`, 0)

# Lower Bound for Median Wage by Gender by region
lb_median_1996_region <- aggregate(ingreso_ocup_principal ~ sexo + region, data = wage_by_gender_1996, FUN = lb_median)
setnames(lb_median_1996_region, colnames(lb_median_1996_region), c("sexo","region","1996"))
lb_median_1996_region$`1996` <- as.numeric(lb_median_1996_region$`1996`)
lb_median_1996_region$`1996` <- round(lb_median_1996_region$`1996`, 0)
lb_median_1996_region$`1996` <- as.numeric(lb_median_1996_region$`1996`)
lb_median_1996_region$`1996` <- round(lb_median_1996_region$`1996`, 0)

# Upper Bound for Median Wage by Gender by region
ub_median_1996_region <- aggregate(ingreso_ocup_principal ~ sexo + region, data = wage_by_gender_1996, FUN = ub_median)
setnames(ub_median_1996_region, colnames(ub_median_1996_region), c("sexo","region","1996"))
ub_median_1996_region$`1996` <- as.numeric(ub_median_1996_region$`1996`)
ub_median_1996_region$`1996` <- round(ub_median_1996_region$`1996`, 0)

# Lower Bound for Mean Wage by Gender by region
lb_mean_1996_region <- aggregate(ingreso_ocup_principal ~ sexo + region, data = wage_by_gender_1996, FUN = lb_mean)
setnames(lb_mean_1996_region, colnames(lb_mean_1996_region), c("sexo","region","1996"))
lb_mean_1996_region$`1996` <- as.numeric(lb_mean_1996_region$`1996`)
lb_mean_1996_region$`1996` <- round(lb_mean_1996_region$`1996`, 0)

# Upper Bound for Mean Wage by Gender by region
ub_mean_1996_region <- aggregate(ingreso_ocup_principal ~ sexo + region, data = wage_by_gender_1996, FUN = ub_mean)
setnames(ub_mean_1996_region, colnames(ub_mean_1996_region), c("sexo","region","1996"))
ub_mean_1996_region$`1996` <- as.numeric(ub_mean_1996_region$`1996`)
ub_mean_1996_region$`1996` <- round(ub_mean_1996_region$`1996`, 0)
