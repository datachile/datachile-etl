...
p_load(data.table, readr, jsonlite, readxl, dplyr, tidyr, stringr, janitor, combinat, purrr)

candidatos <- read_csv("~/GitHub/datachile-data/official_ids/politics/2018-02-14_candidatos_id.csv") %>% 
  mutate(
    candidato = toupper(iconv(candidato, from = "", to = "ASCII//TRANSLIT", sub = ""))
  )

candidatos_variaciones_2009 <- read_csv("candidatos_variaciones_2009.csv")

candidatos_variaciones_2013 <- read_csv("candidatos_variaciones_2013.csv") %>% 
  mutate(
    candidato = toupper(iconv(candidato, from = "", to = "ASCII//TRANSLIT", sub = ""))
  )
...
listas <- read_csv("~/GitHub/datachile-data/official_ids/politics/2018-02-13_listas_id.csv") %>% 
  mutate(lista = toupper(iconv(lista, from = "", to = "ASCII//TRANSLIT", sub = "")),
         lista = gsub(" \\(.*", "", lista))
...
listas_2009 <- tibble(
  lista = LETTERS[1:4],
  pacto = c("CONCERTACION Y JUNTOS PODEMOS MAS", 
            "COALICION POR EL CAMBIO", 
            "NUEVA MAYORIA PARA CHILE", 
...
d2017 <- read_csv("datos-semi-procesados/d2017_votacion_total.csv") %>% 
  select(comuna_name, candidato, lista, partido, votos_candidato, electo) %>% 
  mutate(
    candidato = toupper(iconv(candidato, from = "", to = "ASCII//TRANSLIT", sub = "")),
    candidato = gsub(".*\\.", "", candidato),
    candidato = gsub("(?<=[\\s])\\s*|^\\s+|\\s+$", "", candidato, perl = TRUE),
    ...
    comuna_name = ifelse(comuna_name == "TILTIL", "TIL-TIL", comuna_name),
    comuna_name = ifelse(comuna_name == "TREGUACO", "TREHUACO", comuna_name),
    lista = toupper(iconv(lista, from = "", to = "ASCII//TRANSLIT", sub = "")),
    lista = gsub(".*\\.", "", lista),
    lista = gsub("(?<=[\\s])\\s*|^\\s+|\\s+$", "", lista, perl = TRUE),
    partido = toupper(iconv(partido, from = "", to = "ASCII//TRANSLIT", sub = "")),
    partido = gsub("(?<=[\\s])\\s*|^\\s+|\\s+$", "", partido, perl = TRUE),
    partido = ifelse(partido == "PAIS", "PARTIDO PAIS", partido),
  ...
  left_join(listas) %>% 
  left_join(partidos) %>% 
  mutate(lista_id = ifelse(candidato == "VOTOS NULOS" | candidato == "VOTOS EN BLANCO", 0, lista_id)) %>% 
  mutate(partido_id = ifelse(candidato == "VOTOS NULOS" | candidato == "VOTOS EN BLANCO", 0, partido_id)) %>%
  select(comuna_datachile_id, candidato_id, lista_id, partido_id, votos_candidato, electo) %>% 
  group_by(comuna_datachile_id, candidato_id, lista_id, partido_id, electo) %>% 
  summarise(votos_candidato = sum(votos_candidato, na.rm = T)) %>% 
  mutate(year = 2017,
         election_id = 4)
...
m2016 <- read_csv("datos-semi-procesados/m2016_votacion_total.csv") %>% 
  ...
  filter(candidato != "TOTAL", !is.na(candidato)) %>% 
  left_join(candidatos, by = c("candidato" = "candidato")) %>% 
  left_join(partidos) %>% 
  mutate(partido_id = ifelse(candidato == "VOTOS NULOS" | candidato == "VOTOS EN BLANCO", 0, partido_id)) %>% 
  left_join(comunas) %>% 
...
p20131 <- read_csv("datos-semi-procesados/p20131_votacion_total.csv") %>% 
  ...
  filter(candidato != "TOTALES", !is.na(candidato)) %>% 
  left_join(candidatos, by = c("candidato" = "candidato")) %>% 
  left_join(comunas) %>% 
  filter(!is.na(comuna_datachile_id)) %>% 
  select(comuna_datachile_id, candidato_id, votos_candidato) %>% 
...
d2013 <- read_csv("datos-semi-procesados/d2013_votacion_total.csv") %>% 
  select(candidato, pacto, partido, comuna_name, electo, votos_candidato) %>% 
  rename(lista = pacto) %>% 
  mutate(
    candidato = toupper(iconv(candidato, from = "", to = "ASCII//TRANSLIT", sub = "")),
    candidato = gsub(".*\\.", "", candidato),
    candidato = gsub("(?<=[\\s])\\s*|^\\s+|\\s+$", "", candidato, perl = TRUE),
    candidato = gsub("ISASI BARBIERI MARTA ELIANA", "MARTA ISASI BARBIERI", candidato),
    candidato = gsub("ESPINOSA MONARDES MARCOS", "MARCOS ANDRES ESPINOSA MONARDES", candidato),
    candidato = gsub("MULET MARTINEZ JAIME", "JAIME MULET MARTINEZ", candidato),
    candidato = gsub("VELASCO RODRIGUEZ ERNESTO", "ERNESTO VELASCO RODRIGUEZ", candidato),
    candidato = gsub("INSUNZA GREGORIO DE LAS HERAS JORGE", "JORGE ENRIQUE INSUNZA GREGORIO DE LAS HERAS", candidato),
    candidato = gsub("NAHUELPI VILLAR ERIC MARCIAL", "ERIC MARCIAL NAHUELPI VILLAR", candidato),
    candidato = gsub("BARROS MONTERO RAMON JOSE", "RAMON BARROS MONTERO", candidato),
    candidato = gsub("AGUILO MELO SERGIO", "SERGIO PATRICIO AGUILO MELO", candidato),
    candidato = gsub("NARANJO ORTIZ JAIME", "JAIME NARANJO ORTIZ", candidato),
    candidato = gsub("MONJE CHAVEZ JORGE", "JORGE OSVALDO MONJE CHAVEZ", candidato),
    candidato = gsub("SILVA NAVARRETE ALEXIS LEONARDO", "ALEXIS LEONARDO SILVA NAVARRETE", candidato),
    candidato = gsub("ESPINOZA BROWER ARTURO", "ARTURO EDUARDO ESPINOZA BROWER", candidato),
    candidato = gsub("ANCALAO GAVILAN DIEGO", "JOSE ANCALAO GAVILAN", candidato),
    candidato = gsub("VIVANCO MANRIQUEZ LUIS FERNANDO", "LUIS FERNANDO VIVANCO MANRIQUEZ", candidato),
    candidato = gsub("GONZALEZ OJEDA OCTAVIO ENRIQUE", "OCTAVIO GONZALEZ OJEDA", candidato),
    candidato = gsub("ACCORSI OPAZO ENRIQUE", "ENRIQUE LUIS ACCORSI OPAZO", candidato),
    candidato = gsub("VENEGAS ROJAS CLAUDIO", "CLAUDIO MARCELO VENEGAS ROJAS", candidato),
    candidato = gsub("SULE FERNANDEZ ALEJANDRO", "ALEJANDRO ANSELMO SULE FERNANDEZ", candidato),
    candidato = gsub("JARAMILLO BECKER RAMON ENRIQUE", "ENRIQUE JARAMILLO BECKER", candidato),
    candidato = gsub("CUELLAR GODOY CAROLL", "CAROLL ANDREA CUELLAR GODOY", candidato),
    candidato = gsub("VARGAS PIZARRO ORLANDO", "ORLANDO SEVERO VARGAS PIZARRO", candidato),
    comuna_name = toupper(iconv(comuna_name, from = "", to = "ASCII//TRANSLIT", sub = "")),
    comuna_name = toupper(iconv(comuna_name, from = "", to = "ASCII//TRANSLIT")),
    comuna_name = ifelse(comuna_name == "AISEN", "AYSEN", comuna_name),
    comuna_name = ifelse(comuna_name == "ALTO BIOBIO", "ALTO BIO BIO", comuna_name),
  ...
  left_join(candidatos) %>% 
  left_join(candidatos_variaciones_2013, by = "candidato") %>% 
  left_join(candidatos, by = c("permutacion" = "candidato")) %>% 
  mutate(candidato_id = ifelse(is.na(candidato_id.x), candidato_id.y, candidato_id.x)) %>% 
  left_join(comunas) %>% 
  filter(!is.na(comuna_datachile_id)) %>% 
  left_join(listas) %>% 
  left_join(partidos) %>% 
  mutate(lista_id = ifelse(candidato.x == "VOTOS NULOS" | candidato.x == "VOTOS EN BLANCO", 0, lista_id)) %>% 
  mutate(partido_id = ifelse(candidato.x == "VOTOS NULOS" | candidato.x == "VOTOS EN BLANCO", 0, partido_id)) %>% 
  select(comuna_datachile_id, candidato_id, lista_id, partido_id, votos_candidato, electo) %>% 
  group_by(comuna_datachile_id, candidato_id, lista_id, partido_id, electo) %>% 
  summarise(votos_candidato = sum(votos_candidato, na.rm = T)) %>% 
  mutate(year = 2013,
...
s2013 <- read_csv("datos-semi-procesados/s2013_votacion_total.csv") %>% 
  select(candidato, lista, partido, comuna_name, electo, votos_candidato) %>% 
  mutate(
    candidato = toupper(iconv(candidato, from = "", to = "ASCII//TRANSLIT", sub = "")),
    ...
    candidato = gsub("VON BAER JAHN ENA", "ENA VON BAER JAHN", candidato),
    candidato = gsub("(?<=[\\s])\\s*|^\\s+|\\s+$", "", candidato, perl = TRUE),
    comuna_name = toupper(iconv(comuna_name, from = "", to = "ASCII//TRANSLIT", sub = "")),
    comuna_name = toupper(iconv(comuna_name, from = "", to = "ASCII//TRANSLIT")),
    comuna_name = ifelse(comuna_name == "AISEN", "AYSEN", comuna_name),
  ...
  filter(!is.na(comuna_datachile_id)) %>% 
  left_join(listas2) %>% 
  left_join(listas, by = c("pacto" = "lista")) %>% 
  left_join(partidos) %>% 
  mutate(lista_id = ifelse(candidato.x == "VOTOS NULOS" | candidato.x == "VOTOS EN BLANCO", 0, lista_id)) %>% 
  mutate(partido_id = ifelse(candidato.x == "VOTOS NULOS" | candidato.x == "VOTOS EN BLANCO", 0, partido_id)) %>% 
  select(comuna_datachile_id, candidato_id, lista_id, partido_id, votos_candidato, electo) %>% 
...
#   filter(electo == 1) %>%
#   left_join(candidatos) %>%
#   left_join(comunas2 %>% select(comuna_datachile_id, comuna_name, region_id, region_name)) %>%
#   select(comuna_name, comuna_datachile_id, region_name, region_id, candidato) %>%
#   distinct()
...
s2009 <- read_csv("datos-semi-procesados/s2009_votacion_total.csv") %>% 
  ...
  mutate(
    ...
    candidato = gsub(".*\\.", "", candidato),
    candidato = gsub("(?<=[\\s])\\s*|^\\s+|\\s+$", "", candidato, perl = TRUE),
    candidato = gsub("ROSSI CIOCCA FULVIO", "FULVIO ROSSI CIOCCA", candidato),
    comuna_name = toupper(iconv(comuna_name, from = "", to = "ASCII//TRANSLIT", sub = "")),
    comuna_name = toupper(iconv(comuna_name, from = "", to = "ASCII//TRANSLIT")),
    comuna_name = ifelse(comuna_name == "AISEN", "AYSEN", comuna_name),
    comuna_name = ifelse(comuna_name == "ALTO BIOBIO", "ALTO BIO BIO", comuna_name),
  ...
  left_join(candidatos, by = c("candidato" = "candidato")) %>% 
  left_join(candidatos_variaciones_2009, by = c("candidato" = "candidato")) %>% 
  left_join(candidatos, by = c("candidato2" = "candidato")) %>% 
  mutate(candidato_id = ifelse(is.na(candidato_id.y), candidato_id.x, candidato_id.y)) %>% 
  left_join(comunas) %>% 
  filter(!is.na(comuna_datachile_id)) %>% 
  left_join(listas_2009) %>% 
  left_join(listas, by = c("pacto" = "lista")) %>% 
  left_join(partidos) %>% 
  mutate(lista_id = ifelse(candidato.x == "VOTOS NULOS" | candidato.x == "VOTOS EN BLANCO", 0, lista_id)) %>% 
  mutate(partido_id = ifelse(candidato.x == "VOTOS NULOS" | candidato.x == "VOTOS EN BLANCO", 0, partido_id)) %>% 
