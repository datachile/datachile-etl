...
listado_candidatos <- read_excel("ids-incompletos/listado_parcial_candidatos.xlsx")

listado_candidatos_nd <- listado_candidatos %>% 
  distinct(candidato, .keep_all = T) %>% 
  fwrite("ids-incompletos/listado_candidatos_parcial_sin_duplicados.csv")
...
d2013_votacion_total <- d2013_votacion_total %>%
  ...
  select(comuna, candidato_a, partido, votos_totales, electo) %>% 
